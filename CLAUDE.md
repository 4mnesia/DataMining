...
- **Solo técnicas estándar**: `LinearRegression`, `LogisticRegression`, `DecisionTree`, `SVR`/`SVC`, `GaussianNB`, `KMeans`. Prohibido RandomForest, XGBoost, redes neuronales, Prophet, AutoML.
...
**Modelos**: **regresión** (temperatura diaria, R²≈0.93, hiperparámetros por CV con heatmaps en 4.1.2), **clasificación** de lluvia (`llueve` F1≈0.76 / `lluvia_intensa` F1≈0.52; 3 modelos Logística/SVM/Árbol × 3 versiones; desbalance manejado con `class_weight='balanced'` + **ajuste de umbral** en validación + features `nasa_precip` y lags atmosféricos), y **cluster** K-Means *(pendiente de armar, 4.3)*. Métricas de clasificación incluyen accuracy y **balanced_accuracy** (clave por el desbalance de `lluvia_intensa`).
