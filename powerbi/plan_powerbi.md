...
**Fuente:** `panel_powerbi.csv` (un archivo, todo el dataset tratado: 36 columnas, 0 nulos).
**Medidas:** las de `medidas_dax.md` (las de desempeño filtran `conjunto = "test"`).

> **El dashboard es de RESULTADOS y DECISIONES, no de diagnóstico de modelos.**
> Responde "¿qué nos dice el clima de Chile y qué hacemos con eso?". La validación técnica
> de los modelos (ROC, matriz de confusión, dendrograma, codo) vive en el notebook (Fase 5);
> aquí solo va, opcional, un **anexo técnico** al final.

Dos tipos de visual:
- 🟦 **Nativo** = visual estándar de Power BI con campos/medidas (mapas, barras, líneas, KPIs).
- 🐍 **Visual de Python** = *Objeto visual de Python* que ejecuta matplotlib con los datos en vivo. **No son fotos**: se filtran y recalculan con los segmentadores del informe.

### Cómo usar el visual de Python (una vez)
1. *Archivo → Opciones → Scripting de Python* → apuntar al **mismo entorno que usas en el notebook** (tiene `pandas`, `matplotlib`, `numpy`, `scikit-learn`, `scipy`). Si falta uno, el visual sale en blanco.
2. *Insertar → Objeto visual de Python*. Arrastrar los campos indicados al área **Valores**.
3. Power BI entrega esos campos como un DataFrame llamado **`dataset`** (quita filas duplicadas → por eso se incluye `codigo_estacion` y `fecha` cuando hace falta conservar todas las filas).
4. Pegar el código y *Ejecutar*. Filtrar la página a `conjunto = "test"` donde se indique.

---

## Página 1 — Resumen ejecutivo (el clima de Chile de un vistazo)
| Visual | Tipo | Campos / Medidas |
|---|---|---|
| KPI: temperatura media nacional | 🟦 Tarjeta | `temp_media` (Promedio) |
| KPI: precipitación anual media | 🟦 Tarjeta | `precip_anual` (Promedio) |
| KPI: Nº de estaciones | 🟦 Tarjeta | `codigo_estacion` (Recuento distinto) |
| KPI: Nº de zonas climáticas | 🟦 Tarjeta | `zona_koppen_codigo` (Recuento distinto) |
| KPI credibilidad: acierto del pronóstico | 🟦 Tarjeta | `Accuracy Lluvia` (filtra test) |
| KPI credibilidad: error de temperatura | 🟦 Tarjeta | `RMSE Temp` (filtra test) |
| Mapa de estaciones | 🟦 Mapa | Lat=`latitud`, Long=`longitud`, Leyenda=`cluster_koppen_dominante`, Tamaño=`precip_anual` |
| Texto | 🟦 | "Sistema que segmenta el clima de Chile, pronostica lluvia y estima temperatura para apoyar la gestión hídrica y agrícola." |

## Página 2 — ¿Dónde y cuándo llueve? (resultado)
| Visual | Tipo | Campos / código |
|---|---|---|
| Precipitación por macrozona | 🟦 Barras | Eje=`macrozona`, Valor=`precip_anual` (Promedio) |
| Estacionalidad de la lluvia | 🟦 Columnas | Eje=`mes`, Valor=`agua_caida_tratada` (Promedio) |
| Acierto del pronóstico por zona | 🟦 Barras | Eje=`zona_koppen_codigo`, Valor=`Accuracy Lluvia` |
| **Acierto del pronóstico por mes** | 🐍 Python | Valores: `codigo_estacion`, `fecha`, `mes`, `acierto_lluvia` · filtrar test |

## Página 3 — Temperatura (resultado)
| Visual | Tipo | Campos / código |
|---|---|---|
| Real vs predicha en el tiempo | 🟦 Líneas | Eje=`fecha`, Valores=`temp_media`, `temp_pred` · filtrar test |
| Temperatura por macrozona | 🟦 Barras | Eje=`macrozona`, Valor=`temp_media` (Promedio) |
| KPIs | 🟦 Tarjetas | `R2 Temp`, `RMSE Temp`, `MAE Temp` |
| **Precisión por macrozona** | 🐍 Python | Valores: `codigo_estacion`, `fecha`, `macrozona`, `temp_error_abs` · filtrar test |

## Página 4 — Segmentación climática (resultado del cluster)
| Visual | Tipo | Campos / código |
|---|---|---|
| **Mapa de clusters** | 🐍 Python | Valores: `codigo_estacion`, `latitud`, `longitud`, `cluster_grupo`, `cluster_koppen_dominante` |
| **Perfil de cada cluster** | 🐍 Python | Valores: `codigo_estacion`, `cluster_grupo`, `precip_anual`, `temp_media`, `aridez` |
...
## Anexo técnico (OPCIONAL — solo si se quiere mostrar la validación de los modelos)
| Visual | Tipo | Campos / código |
|---|---|---|
| Curva ROC | 🐍 Python | `codigo_estacion`, `fecha`, `llueve`, `llueve_score` · filtrar test |
| Matriz de confusión | 🐍 Python | `codigo_estacion`, `fecha`, `llueve`, `llueve_pred` · filtrar test |
| Reales vs predichos (temp) | 🐍 Python | `codigo_estacion`, `fecha`, `temp_media`, `temp_pred` · filtrar test |
| Distribución de residuos | 🐍 Python | `codigo_estacion`, `fecha`, `temp_media`, `temp_pred` · filtrar test |
| Dendrograma de estaciones | 🐍 Python | `codigo_estacion`, `precip_anual`, `temp_media`, `aridez` |
| Codo + silhouette | 🐍 Python | `codigo_estacion`, `precip_anual`, `temp_media`, `aridez` |

---

# Código de los visuales de Python (copiar/pegar)

> En todos: el DataFrame de entrada se llama `dataset` y se termina con `plt.show()`.
> Todos probados contra el CSV real y bajo el límite de 150.000 filas por visual.

## Visuales de RESULTADO (los del dashboard)

### Acierto del pronóstico de lluvia por mes  (filtrar a `conjunto = "test"`)
```python
...
### Precisión de la regresión de temperatura por macrozona  (filtrar a `conjunto = "test"`)
```python
import matplotlib.pyplot as plt
...
### Perfil climático de cada cluster — insight para las acciones  (sin filtro de conjunto)
```python
import matplotlib.pyplot as plt
...
## Visuales del ANEXO técnico (opcionales, validación de modelos)

### Curva ROC  (filtrar a `conjunto = "test"`)
```python
import matplotlib.pyplot as plt
...
- Para que la ROC funcione está la columna **`llueve_score`** (probabilidad del modelo) en el CSV.
...
- Para gráficos de variables tratadas (temperatura, lluvia mm, humedad, etc.) NO filtres por conjunto: esas columnas están completas en todas las filas. Solo las de desempeño (`acierto_lluvia`, `temp_error_abs`, predicciones) se filtran a `test`.
