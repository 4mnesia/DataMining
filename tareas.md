...
|---|---|---|---|
| Regresión | SVR RBF | R²=0.933, RMSE=1.39°C | Bueno. Sin sobreajuste (brecha train-test ≈0). |
...
- [x] **Temperatura: tratamiento de errores de sensor (Fase 2.4.3).** El dato crudo DMC traía lecturas imposibles en las colas (`temp_min` de −49.9 °C en estaciones templadas como Rancagua; `temp_media` de −7 °C en agosto). Se trató análogo a la lluvia: toda lectura que se aparta más de 15 °C de la mediana de su estación en ese mes se reemplaza por esa mediana (el umbral supera el p99.9 de la variación real, ~11–14 °C, así que no borra eventos meteorológicos). Resultado: `rango_termico` máx ~34 °C (antes 80 °C), `temp_min` mín −18.8 °C, target de regresión más limpio (R² 0.932→0.933). `temp_media` es el único modelo afectado (regresión); clasificación/cluster no usan estas columnas.
